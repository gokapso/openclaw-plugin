...
function buildMessageFacts(event: KapsoInboundEvent) {
  return {
...
function buildOpenClawMedia(event: KapsoInboundEvent) {
  return event.media.flatMap((media) => {
    if (!media.url) return [];
    if (event.transcript && (event.type === "audio" || media.kind === "audio")) return [];
...
function resolveAdmission(account: ResolvedKapsoAccount, from: string) {
  if (!account.enabled) {
    return { kind: "drop" as const, reason: "kapso account disabled", recordHistory: false };
  }
  if (account.dmSecurity === "disabled") {
    return { kind: "drop" as const, reason: "kapso DM ingress disabled", recordHistory: false };
  }
  if (account.dmSecurity === "allowlist" && account.allowFrom.length > 0 && !isAllowedSender(account.allowFrom, from)) {
    return { kind: "drop" as const, reason: "sender is not in kapso allowFrom", recordHistory: false };
  }
  return undefined;
}

function isAllowedSender(allowFrom: string[], from: string): boolean {
  return allowFrom.some((entry) => entry === from || whatsAppTargetsEquivalent(entry, from));
}

function createInboundDeliveryAdapter(cfg: OpenClawConfig, account: ResolvedKapsoAccount, event: KapsoInboundEvent) {
  return {
    durable: {
      to: event.from,
      replyToId: event.messageId,
...
function listPayloadMediaUrls(payload: ReplyPayload): string[] {
  const urls = new Set<string>();
  const record = payload as { mediaUrl?: unknown; mediaUrls?: unknown };
  if (typeof record.mediaUrl === "string" && record.mediaUrl) urls.add(record.mediaUrl);
