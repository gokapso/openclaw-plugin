...
import {
  applyKapsoAccountConfig,
  applyKapsoAccountName,
  inspectKapsoAccount,
  kapsoChannelConfigSchema,
  listKapsoAccountIds,
  resolveDefaultKapsoAccountId,
  resolveKapsoAccount,
  setKapsoAccountEnabled,
  validateKapsoSetupInput
} from "./config.js";
...
export const kapsoWhatsappPlugin: ChannelPlugin<ResolvedKapsoAccount> = {
  id: CHANNEL_ID,
  meta: {
    id: CHANNEL_ID,
    label: CHANNEL_LABEL,
  ...
  setup: {
    ...
    validateInput: ({ input }) => validateKapsoSetupInput(input as Record<string, unknown>),
...
function buildKapsoAccountSnapshot(account: ResolvedKapsoAccount) {
  return {
...
function inspectAccountDetails(account: ResolvedKapsoAccount): Record<string, unknown> {
  return {
    accountId: account.accountId,
