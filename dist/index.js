...
import { inspectKapsoAccount, listKapsoAccountIds, normalizeWebhookPath, resolveDefaultKapsoAccountId, resolveKapsoAccount } from "./config.js";
...
async function checkWebhookRegistration(account) {
...
async function listWebhooksWithApi(params) {
    ...
    try {
        const url = new URL(`${normalizeKapsoPlatformBaseUrl(params.baseUrl)}/platform/v1/whatsapp/phone_numbers/${encodeURIComponent(params.phoneNumberId)}/webhooks`);
...
function readWebhookList(value) {
    ...
    return source.map(readWebhookSummary).filter((webhook) => Boolean(webhook));
}
function readWebhookSummary(value) {
...
function sameWebhookPath(rawUrl, expectedPath) {
...
function readStringArray(value) {
    if (Array.isArray(value)) {
        return value.map(readCliString).filter((entry) => Boolean(entry));
    }
...
function formatWebhookRegistration(registration) {
    const parts = [
        registration.status,
